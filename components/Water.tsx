...
// Removed unused Button import to prevent warnings
...
export const Water: React.FC<WaterProps> = ({ user, language, onToggleWater, currentHour }) => {
  ...
  // ✅ CRITICAL FIX: Safe initialization for Android WebViews
  // This prevents the "White Screen" crash if Notification API is missing
  ...
  const requestNotification = () => {
    ...
    if (!('Notification' in window)) {
      alert("Notifications are not supported in this app version.");
      return;
    }
  ...
  return (
    <div className="p-4 space-y-6 pb-24"> {/* Added pb-24 for bottom nav spacing */}
      {/* Header */}
      ...
      {/* Notification prompt if not enabled (and strictly supported) */}
      {typeof window !== 'undefined' && 'Notification' in window && notificationPermission !== 'granted' && (
        <div 
          onClick={requestNotification}
          className="bg-blue-50 border border-blue-100 p-3 rounded-xl flex items-center gap-3 cursor-pointer hover:bg-blue-100 transition-colors"
        >
          <Info className="text-blue-500" size={20} />
          <span className="text-sm text-blue-700 font-medium">
             {language === Language.AMHARIC ? "ማስታወሻ እንዲደርስዎ ይፍቀዱ" : "Enable notifications for reminders"}
          </span>
        </div>
      )}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       {/* Only show reminder text if translations exist */}
                       {isCurrent && !isCompleted && t.reminderBody && (
